...
const Menu = () => {
  ...
  useGSAP(() => {
    ...
    // Parallax effect for left leaf (moves downward)
    gsap.to('#m-left-leaf', {
        y: -200, // Moves down 150px
        ease: 'none', // Linear movement
        scrollTrigger: {
            trigger: '#menu', // Trigger when #menu enters viewport
            start: 'top bottom', // Start when top of #menu hits bottom of viewport
            end: 'bottom top', // End when bottom of #menu hits top of viewport,
            scrub: 1, // Smooth scrubbing (1-second delay)
        }   
    })
    gsap.to('m-right-leaf', {
        y: 200, // Moves up 150px
        ease: 'none',
        scrollTrigger: {
            trigger: '#menu',
            start: 'top bottom',
